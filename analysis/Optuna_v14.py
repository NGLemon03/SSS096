...
from SSSv095b2 import backtest_unified, compute_single, compute_dual, compute_RMA, compute_ssma_turn_combined
...
def run_backtest(strat, params, df_price, df_factor, cache_dir):
    if strat == "ssma_turn":
        ...
        df_ind, buy_dates, sell_dates = compute_ssma_turn_combined(df_price, df_factor, **ssma_params, cache_dir=str(cache_dir))
        if df_ind.empty:
        ...
        if strat == 'single':
            df_ind = compute_single(df_price, df_factor, params['linlen'], params['factor'], params['smaalen'], params['devwin'], cache_dir=str(cache_dir))
        elif strat == 'dual':
            df_ind = compute_dual(df_price, df_factor, params['linlen'], params['factor'], params['smaalen'], params['short_win'], params['long_win'], cache_dir=str(cache_dir))
        elif strat == 'RMA':
            df_ind = compute_RMA(df_price, df_factor, params['linlen'], params['factor'], params['smaalen'], params['rma_len'], params['dev_len'], cache_dir=str(cache_dir))
        else:
            return np.nan, np.nan, np.nan, 0
        if df_ind.empty:
            return np.nan, np.nan, np.nan, 0
...
def objective(trial):
    ...
    train_ret, train_sharpe, train_mdd, train_trades = run_backtest(strat, params, df_price_train, df_factor_train, CACHE_DIR)
    
    # 載入 OOS
    df_price_oos, df_factor_oos = load_data(TICKER, start_date=OOS_START, end_date=OOS_END, smaa_source='Self')
    oos_ret, oos_sharpe, oos_mdd, oos_trades = run_backtest(strat, params, df_price_oos, df_factor_oos, CACHE_DIR)
    ...
    if np.isnan(train_ret) or np.isnan(oos_ret):
        ...
        if strat == "ssma_turn":
            calc_keys = ['linlen', 'factor', 'smaalen', 'prom_factor', 'min_dist', 'buy_shift', 'exit_shift', 'vol_window', 'quantile_win', 'signal_cooldown_days']
            ssma_params = {k: v for k, v in params.items() if k in calc_keys}
            df_ind, buy_dates, sell_dates = compute_ssma_turn_combined(df_price_train, df_factor_train, **ssma_params, cache_dir=str(CACHE_DIR))
            if not df_ind.empty:
            ...
            if strat == 'single':
                df_ind = compute_single(df_price_train, df_factor_train, params['linlen'], params['factor'], params['smaalen'], params['devwin'], cache_dir=str(CACHE_DIR))
            elif strat == 'dual':
                df_ind = compute_dual(df_price_train, df_factor_train, params['linlen'], params['factor'], params['smaalen'], params['short_win'], params['long_win'], cache_dir=str(CACHE_DIR))
            elif strat == 'RMA':
                df_ind = compute_RMA(df_price_train, df_factor_train, params['linlen'], params['factor'], params['smaalen'], params['rma_len'], params['dev_len'], cache_dir=str(CACHE_DIR))
            else:
                train_result = None
