...
def pick_topN_by_diversity(trials, param_keys, top_n=5, pct_threshold=25):
    """
    基於多樣性的topN試驗選擇
    
    Args:
        trials: 試驗列表，每個試驗包含score和parameters
        param_keys: 用於計算距離的參數鍵
        top_n: 最終選取的試驗數量
    ...
    for tr in trials_sorted:
        ...
        vec = np.array([params.get(k, 0) for k in param_keys])
        ...
        # 如果最小距離大於門檻，則納入
        if min_dist >= np.percentile(dists, pct_threshold):
            vectors.append(vec)
            chosen.append(tr)
...
for strategy in optuna_results['strategy'].unique():
    for data_source in optuna_results['data_source'].unique():
        ...
        diverse_trials = pick_topN_by_diversity(
            ...
            important_params, 
            top_n=5, 
            pct_threshold=25  # 25%百分位數作為距離門檻
        )
        ...
        for name, result in results.items():
            if 'equity_curve' in result and not result['equity_curve'].empty:
                ...
                if not trades_df.empty and 'return' in trades_df.columns:
                    ...
                    result['metrics']['win_rate'] = len(winning_trades) / len(trades_df) if len(trades_df) > 0 else 0
                else:
