...
import SSSv095a2 as SSS  # 核心計算與回測
...
log_file = cfg.LOG_DIR / f'optuna_5a_{TIMESTAMP}.log'
logging.basicConfig(
    level=logging.INFO,
...
def _backtest_once(strat: str, params: dict, trial_results: list, data_source: str) -> tuple[float, int, float, float, float]:
    ...
    ind_p["smaa_source"] = data_source  # ★5a:確保傳遞 smaa_source
    try:
        if strat == "ssma_turn":
        ...
        # ★ 5a:檢查有效數據比例
        if len(df_ind) / len(df_price) < 0.5:  # 有效數據少於 50%
            logging.info(f"試驗被剔除:有效數據比例過低,策略: {strat}, 數據源: {data_source}, 有效數據: {len(df_ind)}/{len(df_price)}, linlen: {params['linlen']}, smaalen: {params['smaalen']}")
            log_to_results("Trial Excluded", f"有效數據比例過低,策略: {strat}, 數據源: {data_source}, 有效數據: {len(df_ind)}/{len(df_price)}, linlen: {params['linlen']}, smaalen: {params['smaalen']}")
            return -np.inf, 0, 0.0, 0.0, 0.0
