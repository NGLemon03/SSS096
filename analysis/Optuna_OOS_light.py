...
def run_backtest(strat, params, df_price, df_factor, cache_dir):
    if strat == "ssma_turn":
        ...
        df_ind, buy_dates, sell_dates = compute_ssma_turn_combined(df_price, df_factor, **ssma_params, cache_dir=cache_dir)
        if df_ind.empty:
        ...
        if strat == 'single':
            df_ind = compute_single(df_price, df_factor, params['linlen'], params['factor'], params['smaalen'], params['devwin'], cache_dir=cache_dir)
        elif strat == 'dual':
            df_ind = compute_dual(df_price, df_factor, params['linlen'], params['factor'], params['smaalen'], params['short_win'], params['long_win'], cache_dir=cache_dir)
        elif strat == 'RMA':
            df_ind = compute_RMA(df_price, df_factor, params['linlen'], params['factor'], params['smaalen'], params['rma_len'], params['dev_len'], cache_dir=cache_dir)
        else:
            return np.nan, np.nan, np.nan
        if df_ind.empty:
            return np.nan, np.nan, np.nan
...
def objective(trial):
    ...
    train_ret, train_sharpe, train_mdd = run_backtest(strat, params, df_price_train, df_factor_train, CACHE_DIR)
    # 載入 OOS
    df_price_oos, df_factor_oos = load_data(TICKER, start_date=OOS_START, end_date=OOS_END, smaa_source='Self')
    oos_ret, oos_sharpe, oos_mdd = run_backtest(strat, params, df_price_oos, df_factor_oos, CACHE_DIR)
